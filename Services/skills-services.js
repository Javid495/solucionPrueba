...
const postData = async(data)=>{
    ...
    let res = await ( await fetch("http://localhost:4000/skills",config)).json();
    console.log(res);
}
const putData = async(data,id)=>{
    config.method = "PUT";
    config.body = JSON.stringify(data);
    let res = await ( await fetch(`http://localhost:4000/skills/${id}`,config)).json();
    console.log(res);
}
...
const searchData = async(data)=>{
    config.method = "GET";
    let res = await ( await fetch(`http://localhost:4000/skills?q=${Object.values(data).join("")}`,config)).json();
    console.log(res);
}
